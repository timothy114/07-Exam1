...
def problem3(point, length, delta, window):
    ...
    point_2 = rg.Point(point.x,point.y+length)
    vertical = rg.Line(point,point_2)
    vertical.thickness = 3
    ...
    for i in range(length // delta + 1):
        point_2 = rg.Point(x_cor+length+(i*20),y_cor)
        line = rg.Line(rg.Point(x_cor,y_cor),point_2)
        line.thickness = 3
        if i % 3 == 0:
            line.color = 'magenta'
        if i % 3 == 1:
