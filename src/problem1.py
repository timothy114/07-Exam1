...
def problem1a(rectangle, square, thickness, window):
    ...
    point_1 = square.center
    rect_center = rectangle.get_center()
    rect_height = rectangle.get_height()
    point_2 = rg.Point(rect_center.x,rect_center.y-(rect_height/2))
...
def problem1b(point, win, width, height, color):
    ...
    center = rg.Point(point.x,point.y+(height/2))
    corner_1 = rg.Point(point.x-(width/2),point.y)
    corner_2 = rg.Point(point.x+(width/2),point.y+height)
